...
# Two layers: sensory and feature layer
# Sensory layer: 32x32 interconnected excitatory neurons
# Feature layer: 32x32 interconnected excitatory neurons, receive 1 sensory neuron input
...
for j in range(image_height + 2):
    ...
    for i in range(image_width + 2):
        row.append(IzhikevichExitatoryNeuron())
    feature_layer.append(row)

for j in range(1, image_height + 1):
    for i in range(1, image_width + 1):
        feature_layer[j][i].register_input(feature_layer[j-1][i-1])
        feature_layer[j][i].register_input(feature_layer[j-1][i])
        feature_layer[j][i].register_input(feature_layer[j-1][i+1])
        feature_layer[j][i].register_input(feature_layer[j][i-1])
        feature_layer[j][i].register_input(feature_layer[j][i+1])
        feature_layer[j][i].register_input(feature_layer[j+1][i-1])
        feature_layer[j][i].register_input(feature_layer[j+1][i])
        feature_layer[j][i].register_input(feature_layer[j+1][i+1])
        feature_layer[j][i].register_input(sensory_layer[j][i])
...
for t in range(time):
    ...
    for j in range(1, image_height+1):
        for i in range(1, image_width+1):
            ...
            feature_layer[j][i].evaluate_state(0)
            feature_layer[j][i].evaluate_synaptic_rule()

            neuron_voltage_list.append(feature_layer[j][i].get_voltage())

    layer_voltage_list.append(neuron_voltage_list)

    # Show network membrane potential
    vol = np.array(neuron_voltage_list).reshape(image_height, image_width)
    pt.set_data(vol)
    plt.draw()
    plt.pause(1e-17)
    ...
    for j in range(0, image_height+2):
        for i in range(0, image_width+2):
            ...
            feature_layer[j][i].update_state()
